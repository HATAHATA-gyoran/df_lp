...
export default class FishingScene {
  mount(container, manager, state, data) {
    ...
    if (typeof window !== 'undefined') {
      ...
      this._debugInfiniteCatch = (sp.get('debug_infinite') === '1'); // Enable infinite boss catching
      const bReq = sp.get('boss_req');
      this._debugBossReq = (bReq != null) ? parseInt(bReq, 10) : null; // Custom touch requirement
      console.log(`[DEBUG] URL Params: kraken_req=${this._krakenReqCount}, boss_req=${this._debugBossReq}, debug_infinite=${this._debugInfiniteCatch}`);
      ...
      window.debugMayoi = () => { console.log('Forces Mayoi'); this._triggerBossEvent?.('mayoi'); };

      // Spawn 100 fish (Debug Swarm)
      window.debugSwarm = (n = 100) => {
        console.log(`[Debug] Spawning ${n} fish...`);
        for (let i = 0; i < n; i++) {
          // Random rank 1-5, simple spawn
          this._spawnFish?.(i % 5 + 1);
        }
      };

      // Unlock All Fish (Debug)
      window.debugUnlockAll = () => {
        console.log('[Debug] Unlocking all fish...');
        try {
          const all = this._fishData || [];
          if (!this.state.caught) this.state.caught = [];
          all.forEach(f => {
            if (!this.state.caught.some(c => c.id === f.id)) {
              this.state.caught.push({ id: f.id, rank: f.rank, timestamp: Date.now() });
            }
          });
          SaveManager.save?.(this.state);
          console.log('[Debug] All fish unlocked.');
    ...
    this._switchToGameScroll = () => {
      ...
      try { this._bossTouchCounts = { mega: 0, fuji: 0, cthulhu: 0, kraken: 0, mayoi: 0 }; this._bossTriggered = false; } catch (_) { }
      // バブル終了
      this._stopBubbles();
      // 黒フェードを戻して表示
    ...
    this._applySilhouetteForFish = (fish) => {
      try {
        ...
        // Mayoiaioikurage Bait Hard Mode: Force all silhouettes to fish_db
        if (this.state.bait?.id === 'mayoiaioikurage') {
          if (img.dataset._silType !== 'fish_db') {
            img.onerror = null;
            img.src = this._getAssetPath?.('assets/fish_db.png') || 'assets/fish_db.png';
            img.style.filter = 'none';
            img.dataset._silType = 'fish_db';
          }
          return;
        }
    ...
    this._triggerBossEvent = (type) => {
      ...
      if (type === 'mega') {
        ...
        fishId = 'mayoiaioikurage'; video = 'assets/mayoiaioikurage.mp4'; sfx = null; nameJa = 'マヨイアイオイクラゲ';
      }

      if (fishId) {
        // Set pending narration ID for GET sequence
        this._bossNarrationPendingId = fishId;
    ...
    this._updateFishes = (dt) => {
      ...
      for (let i = this._fishes.length - 1; i >= 0; i--) {
        ...
        if (baitRect && fishRect && this._intersects?.(baitRect, fishRect)) {
          ...
          if (forceCatch || baitRank <= fishRank || (this.state.bait?.id === 'mayoiaioikurage')) {
    ...
    this._startScrollBg = () => {
      ...
      const loop = (t) => {
        ...
        let speed = base * (this._scrollSpeedMul || 1);
        // Mayoiaioikurage Bait: HARD MODE (Max Acceleration/Speed)
        if (this.state.bait?.id === 'mayoiaioikurage') {
          speed *= 2.5; // Always fast
        }
    ...
    this._onGetFish = async (def) => {
      ...
      try {
        ...
        // Reset Deep Mode FORCEFULLY (Fix for "Video not playing/hidden")
        this._isDeepMode = false;
        try { document.body.classList.remove('deep-mode'); } catch (_) { }
        try {
          const bg = this.root.querySelector('.fish-bg') || document.querySelector('.fish-bg');
          if (bg) {
            bg.style.transition = ''; // Disable sync transition to snap back
            bg.style.opacity = '1';
            bg.style.display = 'block'; // Ensure block
            bg.style.zIndex = '4'; // Behind Frame (z=5)
            bg.style.transform = 'scale(1.15)'; // Enlarge slightly
          }
        } catch (_) { }
        this._stopDeepEffects?.();
        // 以降はロープ無効化
        ...
        if (this._videoEl) {
          ...
          // クラーケンまたはボスID判定
          const isBoss = def && (def.id === 'kura-ken' || def.id === 'megarodon' || def.id === 'fujisan' || def.id === 'igyounokami' || def.id === 'mayoiaioikurage');
        ...
        if (def && (def.id === 'kura-ken' || def.id === 'mayoiaioikurage')) {
        ...
        // Mayoiaioikurage Narratives (Video Phase)
        // Mayoiaioikurage Narratives (Video Phase)
        if (def && def.id === 'mayoiaioikurage') {
          // Use conventional method (same as boss intro/get)
          this._startBossNarration?.('mayoiaioikurage_video');
        }
    ...
    this._startBossNarration = async (bossId) => {
      try {
        ...
        } else if (bossId === 'mayoiaioikurage') {
          lines = [
            "俺はマヨイアイオイクラゲ",
            "実在する世界最長の海洋生物",
            "…かもな。",
            "いずれにしても、おめでとう。ゲームはクリアだ",
            "どうだ？迷いながら釣るのは楽しかったろう？",
            "じゃあな、へへっ"
          ];
        } else if (bossId === 'mayoiaioikurage_video') {
          lines = [
            "よう、ようやくようやく会えたな。",
            "待ちくたびれたぜ。楽しみすぎてな"
          ];
    ...
    // Simple Dialogue Helper with Katakata Sound
    this._showSimpleDialogue = (text, duration = 2000) => {
      try {
        const t = document.createElement('div');
        Object.assign(t.style, {
          position: 'fixed', bottom: '20%', left: '50%', transform: 'translateX(-50%)',
          background: 'rgba(0, 0, 0, 0.9)', color: '#fff', padding: '20px 40px',
          borderRadius: '4px', fontSize: '24px', fontWeight: 'bold', zIndex: '2147483647',
          pointerEvents: 'none', opacity: '0',
          whiteSpace: 'nowrap', textAlign: 'center', fontFamily: '"Yu Mincho", serif',
          border: '2px solid rgba(255,255,255,0.8)', boxShadow: '0 8px 24px rgba(0,0,0,0.8)',
          textShadow: '0 2px 4px rgba(0,0,0,1)'
        });
        // Typewriter effect
        let i = 0;
        const speed = 100; // ms per char (Slower typing)
        const loop = () => {
          if (i < text.length) {
            t.innerText += text.charAt(i);
            i++;
            if (i % 2 === 0) this._playSfxOnce?.('assets/katakata.wav'); // Sfx every other char
        ...
        t.innerText = ''; // Clear initial
        document.body.appendChild(t); // Ensure append
        requestAnimationFrame(() => { t.style.opacity = '1'; loop(); });
      } catch (_) { }
    };
      ...
      const onEnd = () => {
        ...
        try {
          ...
          if (v) {
            ...
            v.style.visibility = 'visible';
            try {
              v.parentElement.style.zIndex = '';
              v.parentElement.style.transform = ''; // Reset Scale
            } catch (_) { } // Reset Z-Index
            try { v.play?.(); } catch (_) { }
          }
        } catch (_) { }
        try {
            ...
            try {
              ...
              if (hit) {
                ...
                if (!f._touchedRopeHash) {
                  ...
                  else if (d >= 4000) type = 'mayoi';
                  // Prerequisite for Mayoiaioikurage: Must have caught Marianasnailfish
                  if (type === 'mayoi' && !this._isCaught?.('marianasuneirufisshu')) {
                    type = null;
                  }

                  if (type) {
                    // One-time catch check
                    const bossIdMap = { kraken: 'kura-ken', mega: 'megarodon', fuji: 'fujisan', cthulhu: 'igyounokami', mayoi: 'mayoiaioikurage' };
                    const bossId = bossIdMap[type];
                    if (bossId && this._isCaught?.(bossId) && !this._debugInfiniteCatch) {
                      // Already caught: do not increment count, do not trigger
                    } else {
                      this._bossTouchCounts = this._bossTouchCounts || { kraken: 0, mega: 0, fuji: 0, cthulhu: 0, mayoi: 0 };
                      ...
                      else if (type === 'mayoi') needed = 200;

                      // Override requirement if URL param is present
                      if (this._debugBossReq != null) {
                        needed = this._debugBossReq;
                      }
                      // Ease requirement for Debug Mode (Only if boss_req NOT set)
                      else if (this._debugInfiniteCatch) {
                        needed = 1;
                      }

                      if (this._debugBossReq != null || this._debugInfiniteCatch) {
                        console.log(`[Boss Debug] Touch detected: ${type} (${this._bossTouchCounts[type]}/${needed}) - Debug Active`);
                      } else {
                        console.log(`[Boss Debug] Touch detected: ${type} (${this._bossTouchCounts[type]}/${needed})`);
                      }

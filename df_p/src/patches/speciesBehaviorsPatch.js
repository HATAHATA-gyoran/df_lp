  ...
  if (typeof origApply === 'function') {
    Fish.prototype.applyMovementPreset = function (...args) {
      ...
      try {
        ...
        // akagenge: 上昇しながら横波 + 横に長く
        if (id === 'akagenge') {
          applyWidthScaleOnce(this, 2.0, '_sizeAdjAkagenge');
          this._waveXEnabled = true;
        ...
        // kintokidai: JSON側でウメイロ(円運動)に統一したため、ここは削除(or 無効化)
        // if (id === 'kintokidai') { ... }
        // uchiwafugu: 餌接近で巨大化
        if (id === 'uchiwafugu') {
      ...
      try {
        ...
        if ((now - lastSpecB) >= 100) {
          ...
          for (let i = 0; i < fis.length; i++) {
            ...
            if (fid === 'ooguchiboya' && bx != null) {
              // Enhanced Step Move: Faster interval (120ms) and longer step (80px)
              if (!f._stepNext || now >= f._stepNext) {
                const dx = bx - f.x, dy = by - f.y; const ln = Math.hypot(dx, dy) || 1; const ux = dx / ln, uy = dy / ln;
                const step = 80; f.x += ux * step; f.y += uy * step; f.direction = (ux >= 0) ? 'right' : 'left'; f.setPosition(f.x, f.y);
                f._stepNext = now + 120;
              }
            }
            ...
            if (fid === 'mitsukurizame') {
              // Rapid rotation & Chase
              f._mitsuRot = (f._mitsuRot || 0) + 900 * dsec;
              if (f.el) f.el.style.transform = `translate(-50%, -50%) rotate(${f._mitsuRot}deg) scale(${f.def.movement?.sizeScale || 1})`;
              if (bx != null && by != null) {
                const dx = bx - f.x, dy = by - f.y; const dist = Math.hypot(dx, dy) || 1;
                const speed = 140;
                f.x += (dx / dist) * speed * dsec; f.y += (dy / dist) * speed * dsec; f.setPosition(f.x, f.y);
              }
            }
          }
          lastSpecB = now;

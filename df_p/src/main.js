...
// Debug: Unlock all fish if ?unlock_all=true
if (new URLSearchParams(window.location.search).has('unlock_all')) {
  console.log('[DEBUG] Unlocking all fish...');
  fetch('src/data/fish.json')
    .then(r => r.json())
    .then(data => {
      if (data && data.fish) {
        state.caught = data.fish.map(f => ({
          id: f.id,
          name_ja: f.name_ja,
          rank: f.rank
        }));
        console.log('[DEBUG] All fish unlocked:', state.caught.length);
      }
    });
}

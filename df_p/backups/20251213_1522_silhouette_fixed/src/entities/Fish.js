class Fish {
  ...
  applyMovementPreset() {
    ...
    // 基礎不透明度
    this._baseOpacity = (typeof mv.opacity === 'number') ? Math.max(0, Math.min(1, mv.opacity)) : null;
    ...
    // --- 多角形移動（キチジ、タカノハダイ等） ---
    if ((this.def?.id || '') === 'kichiji' || mv.polyMove) {
  ...
  update(dt) { // dt in seconds
    ...
    if (this._pattern === 'straight') {
      ...
      // 追加: 第二近接魚への追尾
      if (this._seekSecondNearestFish && this._seekOtherSpeed > 0 && this._seekOtherTargetX != null) {

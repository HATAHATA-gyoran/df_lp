...
(function () {
  ...
  FishingScene.prototype.mount = function (container, manager, state, data) {
    ...
    this._spawnFishFromBottom = function (def) {
      ...
      return prevSpawnBottom?.call(self, def);
    };
          ...
          for (let i = 0; i < fis.length; i++) {
            ...
            if (fid === 'inugochi' && bx != null) {
              if (!f._stepNext || now >= f._stepNext) {
                ...
                const step = 160; f.x += ux * step; f.y += uy * step; f.direction = (ux >= 0) ? 'right' : 'left'; f.setPosition(f.x, f.y);
                f._stepNext = now + 250;
              }
            }
            ...
            if (fid === 'kichiji') {
              if (!f._roamK) f._roamK = { tx: null, ty: null, timer: 0, speed: 120 };
              const ro = f._roamK; ro.timer += (now - lastSpecB);
              if (ro.tx == null || ro.ty == null || ro.timer > 2000) { ro.tx = Math.floor(sr.width * (0.08 + Math.random() * 0.84)); ro.ty = Math.floor(sr.height * (0.10 + Math.random() * 0.80)); ro.timer = 0; }
              const dx = ro.tx - f.x, dy = ro.ty - f.y; const dist = Math.hypot(dx, dy) || 1; const ux = dx / dist, uy = dy / dist; const step = ro.speed * dsec; f.x += ux * step; f.y += uy * step; f.direction = (ux >= 0) ? 'right' : 'left'; f.setPosition(f.x, f.y);
              if (!f._trailAt || (now - f._trailAt) >= 140) {
                try {
                  const ghost = document.createElement('img'); ghost.src = f.def?.sprite || '';
                  const r = f.el.getBoundingClientRect(); ghost.style.position = 'absolute'; ghost.style.left = `${Math.floor(f.x)}px`; ghost.style.top = `${Math.floor(f.y)}px`; ghost.style.width = `${Math.max(1, Math.round(r.width))}px`; ghost.style.pointerEvents = 'none'; ghost.style.opacity = '0.35'; ghost.style.filter = 'saturate(0.7) blur(0.3px)'; ghost.style.transition = 'opacity 0.35s linear'; ghost.style.zIndex = '6';
                  self._safe.appendChild(ghost); requestAnimationFrame(() => { try { ghost.style.opacity = '0'; } catch (_) { } }); setTimeout(() => { try { ghost.remove(); } catch (_) { } }, 380);
                } catch (_) { }
                f._trailAt = now;
              }
            }
